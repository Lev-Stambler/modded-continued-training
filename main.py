...
def run_track1(
    ...
    activation_filter_tail_min_chunk_size: int = 0,
    ...
    if lr_schedule not in {"constant", "linear", "cosine"}:
        raise ValueError("--lr-schedule must be one of: constant, linear, cosine")
    if lr_final < 0:
        raise ValueError("--lr-final must be non-negative")
    ...
    if activation_filter_tail_min_chunk_size < 0:
        raise ValueError("--activation-filter-tail-min-chunk-size must be non-negative")
    if activation_filter_tail_min_chunk_size:
        if activation_filter_tail_min_chunk_size & (activation_filter_tail_min_chunk_size - 1):
            raise ValueError("--activation-filter-tail-min-chunk-size must be a power of two")
        if activation_filter_tail_min_chunk_size > activation_filter_chunk_size:
            raise ValueError("--activation-filter-tail-min-chunk-size must be <= activation-filter-chunk-size")
    ...
    if lr_schedule == "constant":
        ...
        if requested_lr_final <= 0.0:
            raise ValueError("--lr-final must be positive when --lr-schedule is linear or cosine")
        if requested_lr_final > lr:
            raise ValueError("--lr-final must be <= --lr for linear or cosine decay")
        lr_final = requested_lr_final
    ...
    activation_filter_lowpass_chunk_sizes: tuple[int, ...] = ()
    if activation_filter == "hadamard-lowpass":
        tail_min_chunk_size = activation_filter_tail_min_chunk_size or activation_filter_chunk_size
        lowpass_chunk_sizes: list[int] = []
        chunk_size = activation_filter_chunk_size
        while chunk_size >= tail_min_chunk_size:
            lowpass_chunk_sizes.append(chunk_size)
            if chunk_size == tail_min_chunk_size:
                break
            chunk_size //= 2
        activation_filter_lowpass_chunk_sizes = tuple(lowpass_chunk_sizes)
    ...
    config = {
        ...
        "lr_final": lr_final,
        "requested_lr_final": requested_lr_final,
        ...
        "activation_filter_tail_min_chunk_size": activation_filter_tail_min_chunk_size,
        "activation_filter_lowpass_chunk_sizes": list(activation_filter_lowpass_chunk_sizes),
    ...
    activation_filter_stats = {
        ...
        "lowpass_chunks": 0,
        "lowpass_tokens": 0,
        "exact_remainder_tokens": 0,
        "exact_remainder_saved_bytes": 0,
    }
    activation_filter_shape_stats: dict[tuple[int, ...], dict[str, int]] = {}
    activation_filter_current_doc_plans: dict[int, dict[str, Any]] = {}
    activation_filter_order_cache: dict[int, tuple[int, ...]] = {}
    activation_filter_basis_cache: dict[tuple[str, torch.dtype, int, int], torch.Tensor] = {}
    activation_filter_index_cache: dict[tuple[str, int, int], torch.Tensor] = {}

    def activation_filter_order_indices(chunk_size: int) -> tuple[int, ...]:
        cached = activation_filter_order_cache.get(chunk_size)
        if cached is not None:
            return cached
        bits = int(math.log2(chunk_size))
        ...
        indices = tuple(
            bit_reverse(sequency ^ (sequency >> 1)) for sequency in range(chunk_size)
        )
        activation_filter_order_cache[chunk_size] = indices
        return indices
    ...
    def activation_filter_stats_fields() -> dict[str, int]:
        return {
            ...
            "activation_filter_doc_spans": activation_filter_stats["doc_spans"],
            "activation_filter_lowpass_chunks": activation_filter_stats["lowpass_chunks"],
            "activation_filter_lowpass_tokens": activation_filter_stats["lowpass_tokens"],
            "activation_filter_exact_remainder_tokens": activation_filter_stats["exact_remainder_tokens"],
            "activation_filter_exact_remainder_saved_bytes": activation_filter_stats[
                "exact_remainder_saved_bytes"
            ],
        }
    ...
    def activation_filter_keep_for_chunk(suffix_numel: int, chunk_size: int) -> int:
        keep = activation_filter_keep_for_suffix(suffix_numel)
        if chunk_size != activation_filter_chunk_size:
            keep = round(keep * chunk_size / activation_filter_chunk_size)
        return max(1, min(chunk_size, keep))

    def activation_filter_basis(
        device: torch.device,
        dtype: torch.dtype,
        chunk_size: int,
        keep: int,
    ) -> torch.Tensor:
        cache_key = (str(device), dtype, chunk_size, keep)
        cached = activation_filter_basis_cache.get(cache_key)
        if cached is not None:
            return cached

        scale = chunk_size**-0.5
        ordered_indices = activation_filter_order_indices(chunk_size)
        rows = [
            [
                scale if (natural_index & column).bit_count() % 2 == 0 else -scale
                for column in range(chunk_size)
            ]
            for natural_index in ordered_indices[:keep]
        ]
        basis = torch.tensor(rows, device=device, dtype=dtype)
        activation_filter_basis_cache[cache_key] = basis
        return basis

    def activation_filter_order_tensor(device: torch.device, chunk_size: int, keep: int) -> torch.Tensor:
        cache_key = (str(device), chunk_size, keep)
        cached = activation_filter_index_cache.get(cache_key)
        if cached is not None:
            return cached
        indices = torch.tensor(
            activation_filter_order_indices(chunk_size)[:keep],
            device=device,
            dtype=torch.long,
        )
    ...
    def set_activation_filter_doc_ids(doc_ids: torch.Tensor, index_device: torch.device) -> None:
        ...
        for token_count in (seq_len, seq_len - 1):
            if token_count > doc_ids_cpu.shape[1]:
                continue
            lowpass_indices_by_chunk_size: dict[int, list[list[int]]] = {
                chunk_size: [] for chunk_size in activation_filter_lowpass_chunk_sizes
            }
            exact_indices: list[int] = []
            doc_spans = 0
            for row_idx in range(doc_ids_cpu.shape[0]):
                row_doc_ids = doc_ids_cpu[row_idx, :token_count].tolist()
                start = 0
                row_offset = row_idx * token_count
                while start < token_count:
                    end = start + 1
                    doc_id = row_doc_ids[start]
                    while end < token_count and row_doc_ids[end] == doc_id:
                        end += 1
                    doc_spans += 1
                    cursor = start
                    while cursor < end:
                        ...
                        if chosen_chunk_size == 0:
                            exact_indices.extend(row_offset + token_idx for token_idx in range(cursor, end))
                            break
                        chunk_base = row_offset + cursor
                        lowpass_indices_by_chunk_size[chosen_chunk_size].append(
                            list(range(chunk_base, chunk_base + chosen_chunk_size))
                        )
                        cursor += chosen_chunk_size
                    start = end

            lowpass_tiers: list[dict[str, Any]] = []
            lowpass_chunks = 0
            lowpass_tokens = 0
            for chunk_size in activation_filter_lowpass_chunk_sizes:
            ...
            plans[token_count] = {
                ...
                "doc_spans": doc_spans,
                "lowpass_chunks": lowpass_chunks,
                "lowpass_tokens": lowpass_tokens,
                "exact_remainder_tokens": int(exact_tensor.numel()),
                "prefix_count": int(doc_ids_cpu.shape[0]),
            }
        activation_filter_current_doc_plans = plans

    def normalized_fwht_token_axis(values: torch.Tensor, chunk_size: int) -> torch.Tensor:
        work = values.clone(memory_format=torch.contiguous_format)
        h = 1
        while h < chunk_size:
            grouped = work.reshape(work.shape[0], chunk_size // (2 * h), 2 * h, work.shape[2])
            left = grouped[:, :, :h, :]
            right = grouped[:, :, h:, :]
            tmp = left.clone()
            left.add_(right)
            right.neg_().add_(tmp)
            h *= 2
        work.mul_(chunk_size**-0.5)
        return work

    def hadamard_lowpass_encode(chunks: torch.Tensor, chunk_size: int, keep: int) -> torch.Tensor:
        if activation_filter_kernel == "fwht":
            transformed = normalized_fwht_token_axis(chunks, chunk_size)
            return transformed.index_select(1, activation_filter_order_tensor(chunks.device, chunk_size, keep))
        basis = activation_filter_basis(chunks.device, chunks.dtype, chunk_size, keep)
        return torch.matmul(basis, chunks)

    def hadamard_lowpass_decode(lowpass: torch.Tensor, chunk_size: int, keep: int) -> torch.Tensor:
        if activation_filter_kernel == "fwht":
            ...
            coefficients.index_copy_(1, activation_filter_order_tensor(lowpass.device, chunk_size, keep), lowpass)
            return normalized_fwht_token_axis(coefficients, chunk_size)
        basis = activation_filter_basis(lowpass.device, lowpass.dtype, chunk_size, keep)
    ...
    def activation_token_view(tensor: torch.Tensor) -> tuple[int, torch.Tensor] | None:
        ...
        if activation_filter_scope == "token-shaped" and tensor.ndim == 2 and micro_batch_size > 1:
            for tokens_per_sequence in (seq_len, seq_len - 1):
                if (
                    activation_filter_lowpass_chunk_sizes
                    and tokens_per_sequence >= activation_filter_lowpass_chunk_sizes[-1]
    ...
    def pack_activation_tensor(tensor: torch.Tensor) -> Any:
        ...
        if not activation_filter_lowpass_chunk_sizes or token_count < activation_filter_lowpass_chunk_sizes[-1]:
            activation_filter_stats["skipped_tensors"] += 1
            return tensor
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False):
            ...
            if plan is None or plan["prefix_count"] != prefix_count:
                activation_filter_stats["skipped_tensors"] += 1
                return tensor
            lowpass_tiers = plan["lowpass_tiers"]
            exact_indices = plan["exact_indices"]
            if not any(tier["indices"].numel() > 0 for tier in lowpass_tiers):
                activation_filter_stats["skipped_tensors"] += 1
                return tensor
            flattened = view.reshape(prefix_count * token_count, suffix_numel)
            lowpass_payloads: list[tuple[int, int, Any]] = []
            lowpass_bytes = 0
            for tier in lowpass_tiers:
                tier_indices = tier["indices"]
                if tier_indices.numel() == 0:
                    continue
                tier_chunk_size = int(tier["chunk_size"])
                tier_keep = activation_filter_keep_for_chunk(suffix_numel, tier_chunk_size)
                chunks = flattened[tier_indices]
                lowpass = hadamard_lowpass_encode(chunks, tier_chunk_size, tier_keep).contiguous()
                lowpass, tier_lowpass_bytes = quantize_lowpass(lowpass)
                lowpass_payloads.append((tier_chunk_size, tier_keep, lowpass))
                lowpass_bytes += tier_lowpass_bytes
            exact = (
                flattened[exact_indices].contiguous()
                if exact_indices.numel() > 0
                else flattened.new_empty((0, suffix_numel))
            )
        ...
        activation_filter_stats["doc_spans"] += int(plan["doc_spans"])
        activation_filter_stats["lowpass_chunks"] += int(plan["lowpass_chunks"])
        activation_filter_stats["lowpass_tokens"] += int(plan["lowpass_tokens"])
        activation_filter_stats["exact_remainder_tokens"] += int(plan["exact_remainder_tokens"])
        activation_filter_stats["exact_remainder_saved_bytes"] += exact_bytes
        ...
        return (
            "hadamard-lowpass-activation-v2",
            lowpass_payloads,
            exact,
            original_shape,
            view_shape,
    ...
    def unpack_activation_tensor(packed: Any) -> torch.Tensor:
        if not (isinstance(packed, tuple) and packed and packed[0] == "hadamard-lowpass-activation-v2"):
            return packed
        (
            _tag,
            lowpass_payloads,
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False):
            plan = activation_filter_current_doc_plans[token_count]
            lowpass_tiers_by_chunk_size = {
                int(tier["chunk_size"]): tier["indices"] for tier in plan["lowpass_tiers"]
            }
            exact_indices = plan["exact_indices"]
            suffix_numel = math.prod(suffix_shape)
            ...
            for tier_chunk_size, tier_keep, lowpass in lowpass_payloads:
                tier_indices = lowpass_tiers_by_chunk_size[tier_chunk_size]
                lowpass = dequantize_lowpass(lowpass, tier_keep)
                restored[tier_indices.reshape(-1)] = hadamard_lowpass_decode(
                    lowpass,
                    tier_chunk_size,
                    tier_keep,
                ).reshape(-1, suffix_numel)
            if exact.numel() > 0:
                restored[exact_indices] = exact
            view = restored.reshape(*prefix_shape, token_count, *suffix_shape)
    ...
    def train_batches():
        ...
        batch_doc_ids: list[torch.Tensor] = []
        next_doc_id = 1
        while True:
            for row in ds:
                text = row.get("text")
                ...
                doc_id_buffer.extend([next_doc_id] * len(ids))
                next_doc_id += 1
                while len(token_buffer) >= seq_len:
                    block = torch.tensor(token_buffer[:seq_len], dtype=torch.long)
                    block_doc_ids = torch.tensor(doc_id_buffer[:seq_len], dtype=torch.long)
                    del token_buffer[:seq_len]
                    del doc_id_buffer[:seq_len]
                    batch.append(block)
                    batch_doc_ids.append(block_doc_ids)
                    if len(batch) == micro_batch_size:
                        yield torch.stack(batch), torch.stack(batch_doc_ids)
                        batch.clear()
                        batch_doc_ids.clear()
    ...
    train_budget_seconds = max(train_deadline - train_loop_start, 1.0e-9)

    def lr_for_step(step_number: int, now: float) -> float:
        if warmup_steps > 0 and step_number <= warmup_steps:
            return lr * step_number / warmup_steps
        if lr_schedule == "constant":
            return lr
        progress = min(max((now - train_loop_start) / train_budget_seconds, 0.0), 1.0)
        if lr_schedule == "linear":
            return lr + (lr_final - lr) * progress
        cosine_weight = 0.5 * (1.0 + math.cos(math.pi * progress))
        return lr_final + (lr - lr_final) * cosine_weight
    ...
    while time.monotonic() < train_deadline and not stopped_reason:
        ...
        for _ in range(grad_accum):
            ...
            try:
                ...
                if not math.isfinite(train_loss_value):
                    event_lr = lr_for_step(step + 1, time.monotonic())
                ...
                if is_cuda_oom(exc):
                    raise_cuda_oom(
                        ...
                        exc,
                        {
                            "step": step + 1,
                            "tokens": tokens,
                            "lr": lr_for_step(step + 1, time.monotonic()),
                        },
                    )
                raise
        ...
        step += 1
        step_lr = lr_for_step(step, time.monotonic())
